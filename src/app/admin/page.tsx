...
export default async function AdminPage() {
  const [overview, feed, analytics] = await Promise.all([
    getAdminOverview(),
    getOperationsFeed(),
    getAnalytics(),
  ]);
                ...
                {analytics.map((item) => (
                  <div key={item.id} className="flex flex-1 flex-col items-center gap-3">
                    <div className="flex w-full flex-1 items-end rounded-lg bg-[#efe2cf] p-2">
                      <div
                        className="w-full rounded-md bg-gradient-to-t from-[#9f5f2a] via-[#c88743] to-[#edc987]"
                        style={{ height: `${Math.max((item.value / maxValue) * 100, 18)}%` }}
                      />
                    </div>
          ...
          <CardContent className="space-y-4">
            <div className="rounded-lg border border-[#eadbc7] bg-[#fdf7ef] p-5">
              ...
              <div className="mt-5 space-y-4">
                {bookingMix.map((item) => (
                  <div key={item.label}>
                    <div className="mb-2 flex items-center justify-between text-sm font-semibold text-[#23180d]">
                      <span>{item.label}</span>
                      <span>{item.value}</span>
                    </div>
                    <div className="h-2.5 rounded-full bg-[#efe2cf]">
                      <div className={`${item.tone} h-2.5 rounded-full`} style={{ width: `${Math.max((item.value / Math.max(overview.bookings.total, overview.inventory.waitlist, 1)) * 100, 14)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
